...
// ══════════════════════════════════════════════════════════════════════
console.log("\n── regression: title lookup / MLPerf base title ──");

test("matches a full title with its base-title-only database record", () => {
  const full = "MLPerf Mobile Inference Benchmark: An Industry-Standard Open-Source Machine Learning Benchmark for On-Device AI";
  const base = "MLPerf Mobile Inference Benchmark";
  assert.strictEqual(lib.lookupTitleSimilarity(full, base), 100);
  assert.strictEqual(lib.lookupTitleVariants(full).includes(base), true);
  assert.strictEqual(lib.lookupTitleSimilarity(full, "MLPerf Desktop Training Benchmark") < 85, true);
});

test("prefers exact/base-title matches over weak unrelated candidates", () => {
  const query = "MLPerf Mobile Inference Benchmark: An Industry-Standard Open-Source Machine Learning Benchmark for On-Device AI";
  const candidates = [
    { title: "MLPerf Desktop Training Benchmark" },
    { title: "MLPerf Mobile Inference Benchmark" },
  ];
  assert.strictEqual(lib.bestMatch(candidates, query).title, candidates[1].title);
});

test("compareEntry treats a base-title record as a real match", () => {
  const original = {
    ENTRYTYPE: "misc",
    ID: "mlperf2020",
    ...
    year: "2022",
    howpublished: "arXiv preprint arXiv:2012.02328",
  };
  const found = {
    title: "MLPerf Mobile Inference Benchmark",
    author: "Reddi, V.",
  ...
  assert.notStrictEqual(cmp.status, "needs_review");
});

// ══════════════════════════════════════════════════════════════════════
console.log("\n── regression: author initials and full names ──");

test("initials are equivalent to full names for comparison", () => {
  assert.strictEqual(lib.compareAuthors("Lane, Nicholas Denis", "Lane, N. D."), 100);
  assert.strictEqual(lib.compareAuthors("Nicholas Denis Lane", "N. D. Lane"), 100);
});

test("mergeMetadata prefers a full author list over initials", () => {
  const primary = { title: "A", author: "Lane, N. D.", _source: "ss" };
  const secondary = { title: "A", author: "Lane, Nicholas Denis", _source: "openalex" };
  const merged = lib.mergeMetadata(primary, secondary);
  assert.strictEqual(merged.author, "Lane, Nicholas Denis");
});

test("completeAuthors never replaces a truncated list with initials", () => {
  const entry = {
    title: "A",
    author: "Vaswani, Ashish and others",
  };
  const found = { title: "A", author: "Vaswani, A. and Shazeer, N." };
  const out = lib.completeAuthors(entry, found);
  assert.strictEqual(out.author, entry.author);
});

test("completeAuthors replaces a truncated list with a full verified list", () => {
  const entry = { title: "A", author: "Vaswani, Ashish and others" };
  const found = { title: "A", author: "Vaswani, Ashish and Shazeer, Noam" };
  const out = lib.completeAuthors(entry, found);
  assert.strictEqual(out.author, found.author);
});

// ══════════════════════════════════════════════════════════════════════
console.log("\n── regression: arXiv and misc export style ──");

test("preserves the user's arXiv @article format", () => {
  const entry = {
    ENTRYTYPE: "article",
    ID: "future2026",
    ...
    doi: "10.48550/arxiv.2607.27231",
    url: "https://arxiv.org/abs/2607.27231",
    howpublished: "arXiv preprint arXiv:2607.27231",
  };
  const out = lib.applyBibStyle(entry);
  assert.deepStrictEqual(Object.keys(out).sort(), ["ENTRYTYPE", "ID", "author", "journal", "title", "year"].sort());
  assert.strictEqual(out.journal, "arXiv preprint arXiv:2607.27231");
  assert.strictEqual(out.howpublished, undefined);
});

test("upgrades a misc entry to inproceedings when a conference record is found", () => {
  const entry = {
    ENTRYTYPE: "misc",
    ID: "mlperf2020",
    title: "MLPerf Mobile Inference Benchmark",
  ...
  const out = lib.upgradeMiscEntry(entry, entry);
  assert.strictEqual(out.ENTRYTYPE, "inproceedings");
  assert.strictEqual(out.booktitle, "MLSys");
  assert.strictEqual(out.journal, undefined);
});

test("upgrades a misc entry to article when a journal record is found", () => {
  const entry = {
    ENTRYTYPE: "misc",
    ID: "example",
  ...
  const found = { ...entry, _type: "journal-article" };
  const out = lib.upgradeMiscEntry(entry, found);
  assert.strictEqual(out.ENTRYTYPE, "article");
  assert.strictEqual(out.journal, "Nat. Mach. Intell.");
  assert.strictEqual(out.booktitle, undefined);
});

test("upgrades a misc entry to an arXiv @article when only arXiv is found", () => {
  const entry = {
    ENTRYTYPE: "misc",
    ID: "preprint2026",
    title: "Example Preprint",
...
test("prefers an existing arXiv howpublished value when upgrading from misc", () => {
  const entry = {
    ENTRYTYPE: "misc",
    ID: "preprint2026",
    title: "Example Preprint",
...
test("extracts LaTeX, BibLaTeX, and pandoc citation keys", () => {
  const keys = lib.extractCitationKeys(String.raw`
    \cite[see][p. 1]{A_1, B}
    \autocite*{C}
    \parencite[see][p. 2]{D}
    \fullcite{E}
    [@F; @G]
  `);
  for (const key of ["a_1", "b", "c", "d", "e", "f", "g"])
    assert.ok(keys.has(key), `Expected ${key} in ${[...keys].join(", ")}`);
});

test("normalizes citation keys case-insensitively", () => {
  const keys = lib.extractCitationKeys("\\cite{Alpha_Beta, GAMMA}");
  assert.ok(keys.has("alpha_beta"));
  assert.ok(keys.has("gamma"));
  assert.strictEqual(keys.size, 2);
});

test("recognizes common project text files", () => {
  assert.strictEqual(lib.isProjectTextFile("paper/main.tex"), true);
  assert.strictEqual(lib.isProjectTextFile("notes.md"), true);
  assert.strictEqual(lib.isProjectTextFile("__MACOSX/paper/main.tex"), false);
  assert.strictEqual(lib.isProjectTextFile(".gitignore"), false);
});
